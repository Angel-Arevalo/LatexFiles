...
\begin{document}
  ...
  \begin{frame}{Uso del principio de comprensión}
    Como vimos, el principio de compresión no es una herramienta válida en la teoría, pero en este punto podemos demostrar un teorema
    que nos dice cuándo podemos aplicarlo.

    \textbf{Teorema 4 (principio débil de comprensión):} Sea $P(x)$ una propiedad y $A$ un conjunto tal que
    para todo $x$, si $P(x)$ entonces $x\in A$ entonces es posible encontrar un conjunto $B$ tal que 
    $B = \{x: P(x)\}$
  \end{frame}
  ...
  \begin{frame}{Axioma del par}
    ...
    Básicamente, se dice que dados dos conjuntos $u$ y $v$, exite el conjunto $\{u, v\}$ 
  \end{frame}
\end{document}
