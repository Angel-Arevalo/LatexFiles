...
\begin{document}
  ...
  \begin{frame}{Proposiciones complejas usadas en la teoría}
    ...
    \begin{frame}{Cuantificadores universales}
      La lógica pretenden envolver una propiedad que cumple ciertos objetos. La forma más fácil de clasificar los elementos es por existencia
      o por totalidad. Por ejemplo, podemos decir que TODAS las personas tienen una madre o que EXISTEN personas que no han ido a Ecuador. Para 
      el desarrollo matemático es necesario detallar lo que vamos a llamar conjunto y una propiedad. 

      Un conjunto es una colección de objetos. Dado un conjunto $A$, decimos que $a$ es un elemento de $A$ y lo simbolizamos con 
      $a\in A$. Notese que $a$ también es un conjunto. Mas adelante veremos que esto no es ninguna contradicción con ayuda del axioma 
      de regularidad o de buena fundación.

      Una propiedad $P(x)$ es una proposición una vez fijada otra proposición. Por ejemplo, tomemos la oración $P(x) = x es falso$,
      y vemos que $P(2 = 3)$ es verdad.
